    ...
    <div class="row">
        <div class="col-md-8 col-md-offset-2">
            <div class="panel panel-default">
                <div class="panel-heading">{{ trans('details.points') }}</div>
                <div class="panel-body">


                    <table class="table table-striped table-bordered table-hover">
                        <tr>
                            <th>{{ trans('details.mission') }}</th>
                            <th>{{ trans('details.points') }}</th>
                            <th>{{ trans('details.comment') }}</th>
                            <th> </th>
                        </tr>
                        @foreach ($all_missions as $mission)
                            ...
                            <form class="form-horizontal" role="form" method="POST" action="{{ url('/awarding') }}">
                                ...
                                <tr class="success">
                                    <td>{{ $mission->mission }} - {{$mission->points}} {{ trans('details.pnt') }}</td>
                                    ...
                                    <td><input id="save_mission" type="submit" class="form-control btn btn-primary" name="save" value="{{trans('details.add_points')}}"/> </td>
                                </tr>
                            </form>
                        ...
                        <h4> {{ trans('details.all') }} {{$monster->points}} {{ trans('details.point') }} </h4>
